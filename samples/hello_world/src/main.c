...
#define PL_UART_DEVICE_NODE DT_NODELABEL(uart16550_0)
// #define PL_UART_DEVICE_NODE DT_NODELABEL(uartlite0)
#define MSG_SIZE 32
K_MSGQ_DEFINE(uart_msgq, MSG_SIZE, 10, 4);
static const struct device *const uart0_dev = DEVICE_DT_GET(UART0_DEVICE_NODE);
static const struct device *const pl_uart_dev = DEVICE_DT_GET(PL_UART_DEVICE_NODE);
...
static void print_pl_uart(char *buf)
{
	int msg_len = strlen(buf);

	for (int i = 0; i < msg_len; i++) {
		uart_poll_out(pl_uart_dev, buf[i]);
	}
}
void serial_lite_cb(const struct device *dev, void *user_data)
{
	uint8_t c;
	if (!uart_irq_update(pl_uart_dev)) {
		return;
	}
	if (!uart_irq_rx_ready(pl_uart_dev)) {
		return;
	}
	/* read until FIFO empty */
	print_pl_uart("echo: ");
	while (uart_fifo_read(pl_uart_dev, &c, 1) == 1) {
		uart_poll_out(pl_uart_dev, c);
	}
	print_pl_uart("\r\n");
}
...
int main(void)
{
	...
	//pl uart
	if (!device_is_ready(pl_uart_dev)) {
		printf("UART device not found!");
		return 0;
	}
	ret = uart_irq_callback_user_data_set(pl_uart_dev, serial_lite_cb, NULL);
	if (ret < 0) {
		if (ret == -ENOTSUP) {
			printf("Interrupt-driven UART API support not enabled\n");
		} else if (ret == -ENOSYS) {
	...
	uart_irq_rx_enable(pl_uart_dev);
	print_pl_uart("\r\nHello! I'm your echo bot.\r\n");
	print_pl_uart("Tell me something and press enter:\r\n");
	// app
	for (int i = 0; i < 20; i++) {
